...
| 在线文档  | `/docs`（Swagger UI）、`/redoc`、`/openapi.json`；由 FastAPI 根据代码自动生成，与 REST 实现同步 |
| 离线文档  | 本文件 `docs/api-reference.md`；gRPC 以 `proto/` 为准 |
...
| `auto_download` | bool   | 否   | 本地无缓存时是否自动从 QMT 下载，默认 `true` |
...
Tick 历史。请求体字段：`symbols`、`start_time`、`end_time`、`fields`、`adjust_type`、`auto_download`（默认 `true`）。
...
| `strategy_name`   | string | 空       | 按策略名精确过滤；不传则返回全部            |

**响应 `data.items[]` 订单字段**（节选）：`order_id`、`stock_code`、`strategy_name`、`order_remark`、`order_status_code` 等。
...
| `volume`        | int    | 数量             |
| `price`         | float  | 价格             |
| `strategy_name` | string | 策略名，传给 QMT；响应中原样返回 |
| `order_remark`  | string | 备注             |


**响应 `data`**：订单对象，含 `order_id`、`strategy_name`、`order_remark`、`order_status_code` 等。
...
| 路径                  | 说明                                                 |
| ------------------- | -------------------------------------------------- |
| `GET /health/`      | 基本健康；返回应用名、版本、模式及 `qmt` 状态（`mock` 为 healthy，dev/prod 未就绪时为 degraded） |
| `GET /health/ready` | 就绪探针。`mock` 恒 200；`dev/prod` 在 QMT（xtdata/xttrader）未就绪时 **503** |
| `GET /health/live`  | 存活探针；含 `pid`、`uptime_seconds`、`heartbeat_age_seconds` 等，供 watchdog 判断僵死 |


`**/health/` 与 `/health/ready` 的 QMT 字段（`data.qmt` 或 `data.checks.qmt`）**


| 子键         | 说明                                                                                    |
| ----------- | ------------------------------------------------------------------------------------- |
| `status`    | `mock` / `healthy` / `not_ready` / `degraded` 等                                       |
| `ready`     | QMT 整体是否就绪；`/health/ready` 据此返回 200/503                                            |
| `xtdata`    | 行情：`mock` / `connected` / `disconnected` / `unavailable` 等                            |
| `xttrader`  | 交易：`mock` / `idle`（无会话但路径已配）/ `connected` / `degraded` / `misconfigured` 等 |
...
| --- | --- |
| `pid` | 进程 ID |
| `started_at_ms` | 启动时间（毫秒时间戳） |
| `last_heartbeat_ms` | 最近心跳时间 |
| `uptime_seconds` | 运行时长（秒） |
| `heartbeat_age_seconds` | 距上次心跳的秒数；持续偏大表示可能僵死 |
...
**`SubmitStockOrderRequest`**（对应 `POST /sessions/{session_id}/orders`）
...
| `stock_code` | string | 标的 |
| `side` | OrderSide | 买卖方向 |
| `price_type` | StockPriceType | 价格类型 |
| `volume` | int64 | 数量 |
| `price` | double | 价格 |
| `strategy_name` | string | 策略名 |
| `order_remark` | string | 备注 |

**`GetStockOrdersRequest`**（对应 `GET /sessions/{session_id}/orders`）

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `session_id` | string | 会话 ID |
| `cancelable_only` | bool | 仅可撤单 |
| `strategy_name` | string | 按策略名精确过滤；空则返回全部 |

**`StockOrder`**（下单响应与查单列表项）
...
| 版本    | 日期         | 说明                                |
| ----- | ---------- | --------------------------------- |
| 1.0.1 | 2026-06-11 | 查单支持 `strategy_name` 过滤；补充 gRPC Trading 消息说明 |
